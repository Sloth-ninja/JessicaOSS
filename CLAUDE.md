...
| Users | `userSettings.ts`, `userApiKeys.ts` (key precedence user > firm > env), `userDataExport.ts`, `userDataCleanup.ts`, `access.ts`, `middleware/auth.ts` (`requireAdmin`, `requireMemberPolicy` — WS8 PR B firm-policy write gate, fail-open), `safeError.ts` (cross-cutting error hygiene: `safeErrorLog`/`safeErrorMessage`/`GENERIC_ERROR_DETAIL`/`failRequest` — redacted server logs + fixed client `detail`, adopted at 56+ route sites) |
