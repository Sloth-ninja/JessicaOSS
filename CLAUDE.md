...
| Firm admin (WS8) | `organisations.ts` (membership, `isAdmin`, `getUserOrganisationId`, member list + `setMemberRole` last-admin guard), `organisationApiKeys.ts` (firm-shared keys CRUD), `apiKeyCrypto.ts` (shared AES-256-GCM for user + firm keys), `usageStats.ts` (PR D: org-scoped usage aggregation for `GET /admin/usage`) |
...
- `src/app/(pages)/` — routes: assistant, projects, tabular-reviews, workflows, company-search / legislation / citation-checker (WS7, "Research" sidebar group), account (models / features / api-keys / connectors / security / privacy-data); `admin/` (WS8 "Firm admin" sidebar group, isAdmin-gated: `dashboard` usage overview — PR D, `firm-settings` members / firm keys / policies — PR B/C); `login`, `signup`, `verify-mfa` at app root.
