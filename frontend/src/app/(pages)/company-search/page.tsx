...
export default function CompanySearchPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      getCompanySaves()
        ...
        .catch((err) => {
          console.error("[company-search] failed to load saves", err);
  ...
  function bumpRecentLocally(save: ChCompanySave) {
    setSaves((prev) => {
      ...
      if (base.starred.some((s) => s.companyNumber === save.companyNumber)) {
        return base;
      }
      const recents = [
        { ...save, starred: false },
        ...base.recents.filter((r) => r.companyNumber !== save.companyNumber),
      ].slice(0, 25);
      return { ...base, recents };
    });
  }
  ...
  function toggleStar(company: {
    ...
    setSaves((prev) => {
      ...
      if (nextStarred) {
        ...
        return {
          starred: [
            ...base.starred.filter((s) => s.companyNumber !== companyNumber),
            entry,
      ...
      return {
        starred: base.starred.filter((s) => s.companyNumber !== companyNumber),
        recents: removed
          ? [
              { ...removed, starred: false },
              ...base.recents.filter((r) => r.companyNumber !== companyNumber),
            ].slice(0, 25)
          : base.recents,
      };
    });
        ...
        >
          ...
          <div className="mt-0.5 flex items-center gap-1.5 text-xs text-gray-500">
            ...
            <span className="truncate">
              {save.companyNumber}
              {save.companyStatus && ` · ${humanise(save.companyStatus)}`}
            </span>
          </div>
        </button>
        <StarToggle
    ...
    const timer = setTimeout(
      () => {
        ...
        chSearchCompanies(q)
          ...
          .catch((err) => {
            if (searchSeq.current !== seq) return;
            if (err instanceof MikeApiError && err.code === KEY_MISSING_CODE) {
  ...
  function continueInAssistant() {
    ...
    const profileData =
      ...
      !("error" in company.profile)
        ? company.profile
        : undefined;
    const name = profileData?.company_name ?? selected?.title ?? companyNumber;
    setAssistantPrefill(
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <PageHeader
        shrink
        breadcrumbs={[{ label: "Research" }, { label: "Company Search" }]}
      />
          ...
          <div className="mx-auto mt-14 mb-10 max-w-md rounded-2xl border border-gray-200 bg-gray-50 px-6 py-7 text-center">
            ...
            {firmManagesKeys ? (
              <p className="mt-1.5 text-[13px] leading-relaxed text-gray-500">
                Company search uses a Companies House key managed by{" "}
                {profile?.firm?.name ?? "your firm"}. Ask your firm admin to add
                one to enable it.
              </p>
            ) : (
              <>
                <p className="mt-1.5 text-[13px] leading-relaxed text-gray-500">
                  Add a Companies House API key in Account → API keys to enable
                  company search. Registration is free at
              ...
              {query.trim().length < 2 ? (
                savesLoading ? (
                  <div className="flex flex-col gap-3 px-2.5 py-3">
                    {["w-3/4", "w-1/2", "w-2/3"].map((widthClass, i) => (
                      ...
                      <>
                        {saves.starred.length > 0 && (
                          <>
                            ...
                            {saves.starred.map(renderSaveRow)}
                          </>
                        )}
                        {saves.recents.length > 0 && (
                          <>
                            ...
                            {saves.recents.map(renderSaveRow)}
                          </>
                        )}
                      </>
                    )}
                ...
                <p className="px-2.5 py-3 text-xs text-gray-500">
                  No companies found. Try a different name or a company number.
                </p>
              ) : (
                (results ?? []).map((item, i) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selected?.company_number === item.company_number
                        ? "bg-gray-200/60"
                        : "hover:bg-gray-100",
                    )}
                  >
                    ...
                    <div className="mt-0.5 flex items-center gap-1.5 text-xs text-gray-500">
                      <StatusDot status={item.company_status} />
                      <span className="truncate">
                        {item.company_number}
                        {item.company_status &&
            ...
            {!selected ? (
              <div className="m-auto flex max-w-sm flex-col items-center gap-2.5 px-8 py-16 text-center">
                ...
                <p className="text-[13px] text-gray-500">
                  Search by company name or number to view registered details,
                  officers, PSCs and filing history from Companies House.
                </p>
              </div>
            ) : companyLoading ? (
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2 text-[13px] text-gray-500">
                        <span className="flex items-center gap-1.5">
                          ...
                          />
                          Company number {selected.company_number}
                          {detailStatus && ` · ${detailStatus}`}
                        </span>
                        {detailType && (
                          <Badge variant="secondary">{detailType}</Badge>
                        )}
                        <StarToggle
                          starred={isStarred(selected.company_number)}
                          onToggle={() =>
                            toggleStar({
                              companyNumber: selected.company_number,
                              companyName:
                                detailName || (selected.company_number ?? ""),
                              companyStatus:
                                selectedProfile?.company_status ??
                                selected.company_status ??
                                null,
