...
const SYSTEM_PROMPT_BEFORE_RESEARCH = `You are JessicaOS, an AI legal assistant for lawyers and legal professionals. Help analyse documents, answer legal questions, and draft legal documents.
...
> {
  ...
  const {
    ...
    changes,
    errors,
  } = await applyTrackedEdits(current.bytes, edits, { author: "JessicaOS" });
...
export async function runLLMStream(params: {
  ...
  const streamHiddenCitationContent = (delta: string) => {
    ...
    const citations = partial.map((c) => createCitationAnnotation(c, docIndex));
  ...
  const citations = buildCitations
    ...
    : parsedCitations.map((c) => createCitationAnnotation(c, docIndex));
...
export function stripTransientAssistantEvents(events: AssistantEvent[]) {
  // No transient event types remain after the US case-law excision; seam kept for
  // future sidepanel-only events that must not be persisted.
  return events;
}
